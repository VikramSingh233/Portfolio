...
const Box = ({title , content , image , sourceCodeUrl }) => {
  return (
    <div className="w-[70%] mx-auto my-10 flex flex-col md:flex-row rounded-lg overflow-hidden shadow-lg bg-[#f6ebe3] transition-all duration-300 hover:shadow-xl hover:scale-102">
      ...
      <button
        // onClick={() => redirect(sourceCodeUrl)}
      ...
      >
        ...
        <span className="font-medium"><Link href={sourceCodeUrl} target="_blank" rel="noopener noreferrer" title="Source Code">View Source Code</Link></span>
      </button>

      {/* Access It Button */}
      {/* <button
        
        onClick={() => redirect(accessUrl)}
        className="flex items-center justify-center gap-2 px-4 py-2 w-full sm:w-auto 
                 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all 
                 duration-300 transform hover:scale-102 shadow-md hover:shadow-lg mt-3"
      >
        <svg 
          className="w-5 h-5" 
        ...
        <span className="font-medium">Access It</span>
      </button> */}
    </div>
      </div>
    </div>
